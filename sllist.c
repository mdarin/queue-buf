...
/*НєЫцЯђЫќДЋЕнЕижЗЕФдЊЫижЎКѓВхШывЛИідЊЫи*/
...
/*ИќаТдЊЫиЃЌаоИФДЂДцдкЬиЖЈЮЛжУЕФЪ§Он*/
...
int SLAword(SLLIST * List, int(*Func)(void *) )
{
  SLLIST *ThisItem;
  int Result = 0;
  ...
  for(ThisItem = List;
      ...
      ThisItem = ThisItem->Next)
  {
    Result = (*Func)( ThisItem->Object );
  }
   return Result;
}

void SLReversal(SLLIST** List){
  SLLIST *p = NULL;
  SLLIST *head = NULL;
  SLLIST *b = NULL;
  head  = *List;
